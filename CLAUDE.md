...
2. **`txmatch.js` — זהות נגזרת, לא מאוחסנת.** אף פונקציה לא קוראת hash שמור;
   ...
   (`txmScorePair`): מדד תשלומים סותר = פסילה, תאריך/ספק/חודש = ניקוד.
...
(כפתור "למה חדשה?" בטבלת הסקירה). מחזיר verdict מתוך
`TXM_VERDICT_LABEL`. הסכום הוא תנאי סף קשיח ולכן הסיבה השקטה הנפוצה —
`txmExplain` מדווח במפורש גם על סכומים **קרובים** (סימן הפוך / הפרש קטן),
בדיוק כי תנאי הסף מסתיר אותם.
